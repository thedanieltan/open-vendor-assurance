...
# Public vendor records expose only the factual model: an accurate
# vendor-published URL, or no URL currently recorded. Internal review,
# quarantine, observation, and repair metadata keep supporting maintenance
# workflows, but vendor maturity/completeness states (for example "complete
# enough for review" or "source coverage incomplete") are not published.
...
EPOCH_PLACEHOLDER_PREFIX = "1970-01-01"


def real_timestamp(value: Any) -> str | None:
    """Return the timestamp string, or None for missing/epoch placeholders.

    A missing timestamp must never surface as January 1, 1970; presentation
    layers render an explicit "snapshot date unavailable" state instead.
    """
    text = str(value or "").strip()
    if not text or text.startswith(EPOCH_PLACEHOLDER_PREFIX):
        return None
    return text
...
def build_meta(pack: dict[str, Any], sources: list[dict[str, Any]], vendor_count: int) -> dict[str, Any]:
    ...
    tag = release_tag()
    date = (
        real_timestamp(commit_date())
        or real_timestamp(source_date(sources))
        or real_timestamp(pack.get("generated_at") or pack.get("generatedAt"))
        or ""
    )
    ...
    return {
        ...
        "pack_generated_at": real_timestamp(pack.get("generated_at") or pack.get("generatedAt")),
...
def build_compiled_catalog(
    ...
    # The catalog interface always exposes the complete controlled vocabulary
    # (with its authoritative human-facing labels), so a source type with no
    # records yet shows zero results instead of silently disappearing.
    labels = source_type_labels()
    source_types = sorted(labels)
    recorded_types = {source.get("source_type") for source in compact_sources if source.get("source_type")}
    unknown_types = sorted(recorded_types - set(source_types))
    if unknown_types:
        raise ValueError(f"catalog records use source types outside the controlled vocabulary: {unknown_types}")
    source_type_counts = {
        source_type: sum(1 for source in compact_sources if source.get("source_type") == source_type)
...
def build_site(
    ...
    write_json(
        output_dir / "data/source-types.json",
        {
            "meta": compiled["meta"],
            "items": compiled["source_types"],
            "labels": compiled["source_type_labels"],
            "counts": compiled["source_type_counts"],
        },
    )
