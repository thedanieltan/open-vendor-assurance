...
def is_applied_plan(path: Path, root: Path = ROOT) -> bool:
    ...
    digest = plan_sha256(path) if path.exists() else None
    for entry in applied_plan_entries(root):
        if entry.get("status") != "applied":
            continue
        if entry.get("plan_sha256") and digest and entry.get("plan_sha256") == digest:
            return True
        if entry.get("plan_path") == relative_path or entry.get("plan_name") == path.name:
            return True
    return False
...
def validate_reviewed_cleanup_plan(path: Path, root: Path = ROOT) -> dict[str, Any]:
...
def select_unapplied_reviewed_plan(root: Path = ROOT) -> Path | None:
    for path in reviewed_plan_paths(root):
        validate_reviewed_cleanup_plan(path, root)
        if not is_applied_plan(path, root):
            return path
    return None
...
def write_env(path: Path, values: dict[str, str | bool]) -> None:
    lines = []
...
def select_command(args: argparse.Namespace) -> int:
    ...
    if requested:
        path = ROOT / requested
        validate_reviewed_cleanup_plan(path)
        ...
        path = select_unapplied_reviewed_plan()
        if path is None:
...
def validate_command(args: argparse.Namespace) -> int:
    ...
    validate_reviewed_cleanup_plan(path)
    print(json.dumps({"plan_path": display_path(path), "plan_sha256": plan_sha256(path)}, indent=2, sort_keys=True))
    return 0
