...
def queue_decision_for(
    ...
    return evaluate_queue(
        str(command_entry.get("queue_lane_id") or command_entry.get("lane_id")),
        load_queue_state(queue_state_path),
    )


def failure_routing_for(
    *,
    lane_id: str | None,
    message: str,
...
def execution_payload(command: str, context_kind: str) -> dict[str, Any]:
    ...
    return {
        "side_effect_class": "hold_state_report",
        "mutates_remote_state": False,
        "mutates_catalog": False,
        "dispatches_workflows": False,
        "label_mutation": {
            ...
            "applied": False,
            "reason": "label mutation is not enabled in WP21 local execution",
        },
        "markdown": "\n".join(
            [
                ...
                "- Applied: `False`",
                "- Remote label mutation is not enabled in WP21 local execution.",
                "",
            ]
        ),
    }
...
def next_safe_action_for(report: dict[str, Any]) -> str:
    if report["decision"] == "executed":
        return "Use the audit report output; no catalog, PR, workflow, or remote label mutation was performed."
...
def execute_command(
    ...
    *,
    ...
    context_kind: str = "comment_thread",
    ...
    if parse_decision["decision"] == "ignored":
        ...
        failure_report = failure_routing_for(
            lane_id=parse_decision.get("lane_id"),
            message="chatops command denied before execution",
            failure_code="permission_policy_denial",
            queue_report=None,
        )
        ...
        if queue_report and queue_report.get("decision") != "allow":
            ...
            failure_report = failure_routing_for(
                lane_id=command_entry.get("lane_id"),
                message="chatops execution blocked by queue decision",
    ...
    report = {
        ...
        "authorization_decision": {
            ...
            "authorized": parse_decision.get("authorized") is True,
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# OpenVA Chat-Ops Execution Report",
        "",
        "## Decision",
        "",
    ...
    if report.get("queue_decision") is not None:
        ...
        lines.extend(
            [
                "## Queue",
                "",
                f"- Lane: `{queue.get('lane_id')}`",
                f"- Decision: `{queue.get('decision')}`",
                f"- Reasons: `{', '.join(queue.get('reasons', []))}`",
                "",
            ]
        )
    if execution.get("markdown"):
        lines.extend(["## Execution Output", "", execution["markdown"]])
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    execute_parser.add_argument("--comment-file", type=Path, required=True)
    execute_parser.add_argument("--actor-role", required=True)
    execute_parser.add_argument("--queue-state", type=Path)
    execute_parser.add_argument("--context-kind", default="comment_thread")
    execute_parser.add_argument("--out", type=Path, default=ROOT / DEFAULT_REPORT)
    execute_parser.add_argument("--out-md", type=Path)
