...
| Cross-caller capability leak via content dedup | There is **no content-derived dedup key**. A SHA-256 of low-entropy vendor names is dictionary-testable, so deduping across callers on request content would let one caller reproduce another's request and obtain a capability to their job/result. Default is a new job per request; the optional idempotency key is high-entropy, scoped to the calling capability, and dedups only that caller's own retries — never across callers, never minting a capability for an existing job to a different caller. |
