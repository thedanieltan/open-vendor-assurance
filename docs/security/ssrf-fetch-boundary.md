...
| Observation lane (`observe-report.yml`, `workflow_dispatch`) | `observe.fetch_public` routes through `build_safe_verify_fetcher` bound to the source vendor's domains; safe-fetch failures map to the `quarantined` observation state (fail closed) | source vendor `official_domains` (from the record's `vendor.yaml`) |
