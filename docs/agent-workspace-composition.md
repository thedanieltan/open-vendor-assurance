...
That schema treats `identity` and `source_references` as first-class required fields
for new agents, not tolerated extension fields. The older `match`, `sources`,
`primary_source_by_type`, and `source_urls_by_type` fields remain in the schema as
