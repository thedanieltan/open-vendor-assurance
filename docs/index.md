# OpenVA Documentation Index

This index helps contributors, maintainers, catalog agents, and downstream consumers find the right OpenVA document quickly.

## Project boundary

Read these first to understand what OpenVA is and is not:

```text
README.md
DISCLAIMER.md
GOVERNANCE.md
CONTRIBUTING.md
SECURITY.md
```

## Architecture and control plane

```text
docs/architecture/OPENVA_SYSTEM_DESIGN.md
docs/architecture/SOURCE_REGISTRY_SCHEMA_V1.md
docs/ci-and-branch-protection.md
docs/continuous-publication.md
docs/compatibility-policy.md
.github/validation-ownership.yaml
docs/operations/contracts/workflow-inventory.yaml
docs/operations/contracts/reviewer-decision-handoff.yaml
docs/maintenance/contracts/catalog-growth-scale-readiness.yaml
```

## Public operation and governance

```text
docs/v0.1.0-public-launch-readiness.md
docs/public-launch-checklist.md
docs/public-launch-cutover.md
docs/continuous-publication.md
docs/compatibility-policy.md
docs/roadmap.md
docs/triage-policy.md
docs/first-good-issue-policy.md
docs/ci-and-branch-protection.md
MAINTAINERS.md
```

## Review operations

```text
docs/human-review-operations.md
docs/source-refinement-workflow.md
docs/observation-reporting.md
docs/source-trust/SOURCE_TRUST_OPERATIONS_RUNBOOK.md
docs/agent-control-plane.md
docs/agent-runbook.md
docs/maintenance/operator-runbook.md
```

## Catalog contribution workflow

```text
docs/submission-intake.md
docs/submission-verification.md
docs/catalog-agent-protocol.md
docs/catalog-batch-generator.md
docs/catalog-reset-2026-05-15.md
docs/breadth-depth-operating-model.md
docs/category-coverage-program.md
docs/category-lane-execution-backlog.md
config/category-taxonomy.yaml
config/region-taxonomy.yaml
docs/region-taxonomy.md
docs/coverage-map.md
docs/coverage-growth.md
config/coverage-targets.yaml
docs/vendor-expansion-backlog.md
docs/public-update-pathway.md
docs/vendor-public-manifest.md
```

## Observation workflow

```text
docs/observation-ledger.md
docs/observation-pilot.md
docs/observation-result-taxonomy.md
docs/source-trust/observation-retention-policy.md
config/observation-pilot.yaml
config/observation-sla.yaml
```

## Consumer and importer workflow

For spreadsheet-first users, use the browser resolver and its exports:

```text
https://thedanieltan.github.io/open-vendor-assurance/
openva-inventory-template.csv
openva-sample-inventory.csv
```

For developers and downstream importers, use:

```text
docs/continuous-publication.md
docs/compatibility-policy.md
docs/agent-export-contract.md
docs/agent-integrations.md
docs/consumer-conformance-fixtures.md
docs/adapter-contract.md
fixtures/packs/
openva-pack.json
indexes/
schemas/openva/
```

For a read-only MCP server over the export contract, see `docs/agent-integrations.md` and `integrations/mcp/openva_mcp/`.

Consumers that require a fixed state should pin an exact repository commit SHA or verified digest. Consumers that follow `main` deliberately receive the latest accepted catalog state.

## Discovery surface

The hosted site exposes a static discovery layer for search engines and machine consumers, generated by `site/build.py` alongside the catalog viewer:

```text
/agents/                     agent and machine integration guide
/vendors/{vendor_id}/        one static page per canonical vendor
/.well-known/openva.json     typed discovery manifest
/sitemap.xml, /robots.txt    crawl surface
/llms.txt                    short pointer file for LLM consumers
```

All OpenVA-owned URLs derive from `config/publication.yaml`.

## Policies

```text
docs/doctrine.md
docs/non-advisory-policy.md
docs/public-sources-only.md
docs/language-policy.md
docs/retention-policy.md
docs/rights-policy.md
docs/continuous-publication.md
docs/compatibility-policy.md
config/controlled-vocabulary.yaml
```

## Validation commands

Every core change should pass:

```bash
python -m tools.openva.validate build-indexes
python -m tools.openva.validate validate
pytest -q
python -m tools.openva.conformance fixtures/packs/minimal-valid
python -m tools.openva.conformance fixtures/packs/valid-bot-protected-observation
python -m tools.openva.conformance fixtures/packs/valid-brand-only-fallback
```

These commands validate the accepted repository and pack state. OpenVA does not prepare or publish formal catalog releases.

## Non-advisory reminder

OpenVA records public-source metadata. It does not approve, recommend, certify, score, or determine whether any vendor is compliant, safe, adequate, suitable, low risk, or high risk.
