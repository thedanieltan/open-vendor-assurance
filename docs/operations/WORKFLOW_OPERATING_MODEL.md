...
| `machine-provisional-materialization.yml` | Scheduled dispatcher for one machine-provisional materialization from the catalog-growth queue. | `workflow_dispatch`, scheduled | `actions: write`, `contents: read` | No | No | No | Dispatch audit via GitHub Actions | `candidate-promotion-pr.yml` machine-provisional mode | Core |
