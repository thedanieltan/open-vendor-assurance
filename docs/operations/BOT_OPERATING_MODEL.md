...
- Level 5 — reversal authority: the WP38b `source_rollback` lane reverts recent machine-created state (promotion, materialization, quarantine) through a PR, never rewrites the ledger or decision history, and must not be the component that authored the state it reverts (reverser != author, machine-enforced by the `rollback_reverser_not_author` release gate).
