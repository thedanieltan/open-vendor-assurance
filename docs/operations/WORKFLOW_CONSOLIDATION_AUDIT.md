...
| `submitted-source-verification.yml` | `keep_support` | Verification-only support path for submission claims: one comment and one candidate label, no catalog writes. | Keep narrowly scoped to the `submission_verification` lane. |
