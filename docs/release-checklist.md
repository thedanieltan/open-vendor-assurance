...
When a maintainer pushes a tag matching `v*`, the `release-downloads` workflow generates non-technical download assets from the tagged commit and attaches them to the GitHub Release:
...
This workflow is the deliberate release-lane exception to the default read-only workflow posture: it uses `contents: write` only to attach assets to the release for the pushed tag. It does not run on ordinary pushes to `main`.

Maintainers should verify that these release assets are present and that `openva-release-downloads-manifest.json` includes checksums for the uploaded files. Maintainers should not manually generate these assets for normal releases.
...
- [ ] `indexes/vendor-match-index.json` is committed and current.
- [ ] `release-artifacts.json` is generated for the release candidate or release asset.
- [ ] GitHub Release download assets were generated by the tag workflow.
