...
## dpmOS Migration Notes

dpmOS should replace local `OPENVA_PACK_PATH` invocation with `OPENVA_SERVICE_URL` and `OPENVA_SERVICE_API_KEY`, send the uploaded CSV to `POST /match`, and remove JSON-string unwrapping for matcher fields because service responses already contain native JSON arrays and objects.
