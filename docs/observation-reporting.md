...
For the v2 observation ledger — latest observations index, freshness vs SLA, changed-since-last-observation, sources-requiring-review, and the committed change-event ledger — see `docs/observation-ledger.md`. Those reports are generated by `source-maintenance-report.yml` from its source-verification output; the legacy dry-run report below remains quarantined and manual-only.
