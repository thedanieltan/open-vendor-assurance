"""Temporary isolated agent-user smoke against current OpenVA main.

This file is never merged. It exits the temporary acceptance run after the smoke
assertions pass so unrelated baseline tests cannot obscure the result.
"""
...
def run_agent_smoke() -> None:
    ...
    meta_body = meta.json()
    assert meta_body["snapshot"]["vendor_count"] > 0
    assert meta_body["snapshot"]["source_count"] > 0
...
run_agent_smoke()
pytest.exit("AGENT_USER_SMOKE_PASS", returncode=0)
