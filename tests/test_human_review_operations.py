...
def test_human_review_operations_defers_dedicated_ui():
    ...
    assert "A separate UI is not required for the current OpenVA phase." in text
...
def test_human_review_operations_defines_roles_and_states():
    ...
    for role in ["Source reviewer", "Language reviewer", "Catalog reviewer", "Release reviewer"]:
        assert role in text
