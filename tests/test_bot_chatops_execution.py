...
NON_EXECUTABLE_COMMANDS = {"/openva retry-source-preflight", "/openva defer-candidate", "/openva promote-reviewed-plan", "/openva quarantine-source", "/openva recheck-final-url"}
...
def test_only_three_approved_commands_are_executable_in_contract():
    ...
    executable = {command["full_command"] for command in contract["commands"] if command["executable"] is True}
...
def test_hold_local_cli_reports_allowed_hold_label_without_remote_mutation():
    report = execute_command("/openva hold", "maintainer", context_kind="pull_request")
...
def test_unhold_local_cli_reports_allowed_hold_label_without_remote_mutation():
    report = execute_command("/openva unhold", "maintainer", context_kind="issue")
    label = report["execution_report"]["label_mutation"]
    assert report["decision"] == "executed"
    assert label["allowed_label"] == "openva-hold"
...
def test_command_execution_cli_writes_json_and_markdown(tmp_path):
    ...
    result = main(["execute", "--comment-file", str(comment), "--actor-role", "maintainer", "--out", str(out_json), "--out-md", str(out_md)])
    assert result == 0
...
def test_execution_layer_does_not_mutate_catalog_data():
    before = data_vendor_digest()
    execute_command("/openva hold", "maintainer")
