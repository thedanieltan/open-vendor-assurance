...
def test_changed_source_observation_guard_requires_latest_baseline(tmp_path):
    source_path = (
        tmp_path / "data" / "vendors" / "example" / "sources" / "example-source.yaml"
    )
...
""".lstrip(),
    ...
    failures = validate_changed_source_observations(
...
def test_catalog_guard_does_not_require_predefined_source_categories(tmp_path):
    ...
    source_path = (
        tmp_path / "data" / "vendors" / "example" / "sources" / "example-security.yaml"
    )
...
""".lstrip(),
    ...
    failures = validate_catalog_batch_duplicates(
...
""".lstrip(),
    ...
    existing_vendor_path = (
        tmp_path / "data" / "vendors" / "existing-vendor" / "vendor.yaml"
    )
