"""Temporary exact browser smoke for the no-match repair; never merged."""
...
def test_repaired_browser_resolves_known_and_unknown_rows_and_exports_csv(tmp_path: Path) -> None:
    site_out = tmp_path / "site"
    subprocess.run([sys.executable, "site/build.py", "--out", str(site_out)], check=True, text=True)
...
const fixesSource = fs.readFileSync(path.join(root, "ui-fixes.js"), "utf8");
...
const context = {
  ...
  document: {
    ...
    createElement: () => ({ click: () => {}, addEventListener: () => {}, dataset: {}, classList: { add: () => {} } }),
  },
  vendorSearch,
  knownDetail,
};
...
vm.runInContext(fixesSource, context);
vm.runInContext(
